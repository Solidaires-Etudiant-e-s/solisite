...
const [{ data: pageData }, { data: siteSettingsData }] = await Promise.all([
  useFetch<CmsPage>('/api/pages/textes-fondamentaux'),
  useFetch<CmsSiteSettings>('/api/site-settings')
])

const page = computed(() => requireCmsData(pageData.value, 'Page Textes fondamentaux introuvable.'))
...
<template>
  <SiteFundamentalTextsPageView :page="page" />
</template>
